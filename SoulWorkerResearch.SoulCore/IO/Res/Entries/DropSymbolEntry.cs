global using DropSymbolKeyType = System.UInt16;
...
[Table(TableName)]
public sealed class DropSymbolEntry : IEntry<DropSymbolEntry>
{
    public const string TableName = "Tb_Drop_Symbol";

    public DropSymbolKeyType Id { get; set; }
    ...
    public DropSymbolEntry(BinaryReader reader)
    {
    ...
    static string IEntry<DropSymbolEntry>.TableName => TableName;

    static DropSymbolEntry IEntry<DropSymbolEntry>.CreateInstance(BinaryReader reader)
    {
        return new DropSymbolEntry(reader);
    }
}
