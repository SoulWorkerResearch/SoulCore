...
public sealed record MonsterSpawnBox : Entity
{
    /// <summary>
    ///
    /// </summary>
    ...
    public MonsterSpawnBox(XElement x) : base(x)
    {
        Monsters = Enumerable
            .Range(1, 10)
            .Select(id => new Monster(x.GetUInt32($"m_iMonsterID{id}"), x.GetEnum<MonsterSpawnType>($"m_eType{id}"), x.GetUInt32($"m_iChance{id}")))
            .ToArray();
